...
int main(void){
    ...
    for (int i = 0; i < 100; i++){
        ...
        if (is_sorted(vi) == false){
            std::cout << "WARNING! L'algoritmo quick sort non ha funzionato correttamente!" << "\n";
            return EXIT_FAILURE;
        }
    ...
    std::vector<std::string> vs = {"astronave", "casa", "laura", "quantitativamente", "balestra", "federico", "cetriolo", "zanzara", "vetta", "riposare"};
            ...
            if (is_sorted(vs) == false){
                std::cout << "WARNING: L'algoritmo quick sort per le stringhe non ha funzionato correttamente!" << "\n";
                return EXIT_FAILURE;
            }

    std::cout << "SUCCESS! L'algoritmo quick sort ha ordinato i vettori correttamente" << "\n";
    return EXIT_SUCCESS;
}